...
def handle_tool_call(function_call):
    ...
    try:
        if function_call.name == "CreateImage":
            ...
            toolResultStr = {"result": f"Unknown function {function_call.name}"}
    except Exception as e:
        toolResultStr = {"result": f"Error executing {function_call.name}: {str(e)}"}
...
if __name__ == "__main__":
    while True:
        ...
        if response.candidates[0].content.parts[0].function_call:
            ...
            toolResultStr = handle_tool_call(function_call)
