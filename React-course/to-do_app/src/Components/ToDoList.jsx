  ...
  return (
    <div className='ToDoList'> 
        ...
        <ul className= {currentScreen === 1 ? "items active" : "items"} id='All'>
            {
                Tasks.map((task)=><li key={task.id}><input onChange={()=>{handleChecker(task)}} checked = {task.completed === true ? true : false} type="checkbox"/> {task.text}</li>)
            }
        </ul>

        <ul className= {currentScreen === 2 ? "items active" : "items"} id='Active'>
            {
                Tasks.filter(task=>task.completed === false).map(task=><li key={task.id}><input onChange={()=>{handleChecker(task)}} checked = {task.completed === true ? true : false} type="checkbox"/> {task.text}</li>)
            }
        </ul>
